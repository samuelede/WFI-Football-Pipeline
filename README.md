...
![WFI Football Pipeline Dashboard](docs/looker_studio_dashboard.png)
...
│   └── looker_studio_dashboard.png  # dashboard screenshot, embedded above
└── data/
...
Once `.env` and `service_account.json` are in place (see Prerequisites and Google Cloud CLI Setup above), the whole pipeline can be started with one command:
 
```bash
bash scripts/run_pipeline.sh
```
 
This builds the Airflow image, brings up all four Docker services, waits for Airflow to actually be ready, confirms both DAGs parsed without errors, unpauses them, and triggers a fresh run. It's the same sequence as the manual steps below, just automated, so it's worth reading Steps 1-7 at least once to understand what each part is doing before relying on the script for repeat runs.
